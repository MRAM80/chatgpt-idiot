        ...
        {pageLoading ? (
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-3xl bg-slate-900 p-6 ring-1 ring-slate-700">
              <h2 className="mb-5 text-base font-bold text-white">Report Filters</h2>
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                <div>
                  ...
                  <select value={driverId} onChange={e => setDriverId(e.target.value)}
                    className="w-full rounded-xl border border-slate-600 bg-slate-800 px-3 py-2.5 text-sm text-white focus:outline-none focus:ring-2 focus:ring-emerald-500">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {[
                  ...
                  <button key={label} type="button"
                    ...
                    className="rounded-lg border border-slate-700 bg-slate-800 px-3 py-1.5 text-xs font-medium text-slate-400 hover:border-emerald-600 hover:text-emerald-400 transition">
                    {label}
                  </button>
                ))}
              </div>
              ...
              <>
                ...
                <div className="grid gap-3 grid-cols-2 sm:grid-cols-4">
                  <div className="rounded-2xl bg-slate-900 p-4 ring-1 ring-slate-700 text-center">
                    ...
                    <div className="mt-1 text-xs font-semibold text-slate-400">Total Orders</div>
                  </div>
                  <div className="rounded-2xl bg-slate-900 p-4 ring-1 ring-emerald-800/50 text-center">
                    <div className="text-3xl font-black text-emerald-400">{totalCompleted}</div>
                    <div className="mt-1 text-xs font-semibold text-slate-400">Completed</div>
                  </div>
                  <div className="rounded-2xl bg-slate-900 p-4 ring-1 ring-slate-700 text-center">
                    <div className="text-3xl font-black text-blue-400">{completionRate}%</div>
                    <div className="mt-1 text-xs font-semibold text-slate-400">Completion Rate</div>
                  </div>
                  <div className="rounded-2xl bg-slate-900 p-4 ring-1 ring-rose-800/50 text-center">
                    <div className="text-3xl font-black text-rose-400">{totalIssues}</div>
                    <div className="mt-1 text-xs font-semibold text-slate-400">Issues Reported</div>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-3">
                  <span className="text-sm text-slate-400">
                    <span className="font-bold text-white">{totalOrders}</span> orders · {dateFrom} → {dateTo}
                    {driverId !== 'all' && <> · <span className="text-white">{driverMap[driverId]}</span></>}
                  </span>
                  <div className="ml-auto flex flex-wrap gap-2">
                    {!showCharts ? (
                      <button onClick={() => setShowCharts(true)}
                        className="rounded-xl border border-emerald-600 bg-emerald-600/10 px-4 py-2 text-sm font-semibold text-emerald-400 hover:bg-emerald-600 hover:text-white transition">
                        Generate Charts
                      </button>
                    ) : (
                ...
                {driverStats.length === 0 ? (
                  ...
                  <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                    {driverStats.map((d, i) => {
                      ...
                      return (
                        <div key={d.id} className="rounded-2xl bg-slate-900 p-5 ring-1 ring-slate-700">
                          {/* Name + rate */}
                          ...
                          <div className="grid grid-cols-3 gap-2 mb-3">
                            <div className="rounded-lg bg-slate-800 py-2 text-center">
                            ...
                            <div className="rounded-lg bg-slate-800 py-2 text-center">
                              <div className="text-lg font-black text-rose-400">{d.issues}</div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid gap-4 lg:grid-cols-2">
                      <div className="rounded-3xl bg-slate-900 p-5 ring-1 ring-slate-700">
                        <h3 className="mb-1 text-sm font-bold text-white">Daily Order Volume</h3>
                        ...
                        <ResponsiveContainer width="100%" height={240}>
                          <LineChart data={dailyData} margin={{ top: 5, right: 10, left: -20, bottom: 5 }}>
                            ...
                            <Tooltip contentStyle={{ background: '#0f172a', border: '1px solid #334155', borderRadius: 8, color: '#f1f5f9', fontSize: 12 }} />
                      ...
                      <div className="rounded-3xl bg-slate-900 p-5 ring-1 ring-slate-700">
                        <h3 className="mb-1 text-sm font-bold text-white">Order Types by Driver</h3>
                    ...
                    <div className="grid gap-4 lg:grid-cols-2">
                      <div className="rounded-3xl bg-slate-900 p-5 ring-1 ring-slate-700">
                        <h3 className="mb-1 text-sm font-bold text-white">Completion Rate by Driver</h3>
                        ...
                        <ResponsiveContainer width="100%" height={240}>
                          <BarChart
                            ...
                            <Tooltip formatter={(v) => `${v}%`} contentStyle={{ background: '#0f172a', border: '1px solid #334155', borderRadius: 8, color: '#f1f5f9', fontSize: 12 }} />
                      ...
                      {driverStats.length === 1 && radarData.length > 0 ? (
                        <div className="rounded-3xl bg-slate-900 p-5 ring-1 ring-slate-700">
                          <h3 className="mb-1 text-sm font-bold text-white">Driver Profile — {driverStats[0].name}</h3>
                        ...
                        <div className="rounded-3xl bg-slate-900 p-5 ring-1 ring-slate-700">
                          <h3 className="mb-1 text-sm font-bold text-white">Daily Output by Driver</h3>
                      ...
                      <table className="w-full divide-y divide-slate-800">
                        <thead className="bg-slate-800">
                          <tr>
                            ...
                            <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-wide text-slate-400">Issues</th>
                          ...
                          {driverStats.map((d, i) => {
                            ...
                            return (
                              <tr key={d.id} className="hover:bg-slate-800/60 transition-colors">
                                ...
                                <td className="px-4 py-3 text-sm text-rose-400">{d.issues}</td>
